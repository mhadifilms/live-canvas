...
A live, local side panel for Codex, Claude Code, Cursor, and OpenCode sessions. Python 3.10+ standard library only; no packages, build step, or Herdr dependency. Local-only mode needs no remote service; TypeSafe focus is optional. The browser reads state once per second using conditional requests; Codex transcript updates are followed every 0.8 seconds. Hidden browser tabs poll every five seconds.
...
The default installs all four clients. Select one with `--client codex|claude|cursor|opencode`; `--client both` selects Claude Code + Cursor. Setup preserves unrelated instructions, hooks, skills, and plugins and refuses edited owned entries.

- **Codex:** A managed block in `~/.codex/AGENTS.md` (or `CODEX_HOME`) requests one persistent canvas on the first user turn. No CLI hook-trust step or trust-record edit is required. Unchanged legacy Live Canvas hooks are removed during upgrade.
- **Claude Code:** Command hooks open the OS browser for foreground sessions and capture visible responses.
- **Cursor:** Hooks request its built-in browser on the first user turn when available, otherwise the OS browser.
- **OpenCode:** A discoverable `~/.config/opencode/plugins/live-canvas.js` plugin opens the OS browser and captures visible messages. Setup migrates owned legacy `.mjs` installs, preserves other plugins, and skips subagent sessions identified by a parent ID.

No adapter can prove visibility from installed files. Verify the actual browser view after starting a foreground task. Native Codex/Cursor panels require an active user turn, not merely clicking a blank-chat button.

Session identities are exact: Codex retains its raw task ID; Claude uses `claude:<session_id>`; Cursor uses `cursor:<conversation_id>` (sessionStart can fall back to `session_id`). OpenCode uses `opencode:<session_id>`. No IDs are inferred from a working directory. SessionStart injects the command prefix, and Cursor also supplies environment values when the host supports them.
...
Claude hooks record generic activity and `Stop.last_assistant_message`; Cursor records generic activity and `afterAgentResponse.text`. Responses arrive at message boundaries rather than token streaming. Visible user messages are captured when supplied by the host. Adapters never read tool inputs/results or thoughts. Codex registers its transcript through the launcher as before. Stopped sessions ignore observations. Malformed input and state contention fail open.
...
Persistent private state defaults to `~/Library/Application Support/Live Canvas` on macOS or `$XDG_DATA_HOME/live-canvas` (normally `~/.local/share/live-canvas`) on Linux. Each task stores a self-contained offline `canvas.html`, a `chat.json` conversation reference, and a bounded `conversation.jsonl` mirror. Existing experimental state is preserved but ignored by this document renderer. Set `SESSION_CANVAS_HOME` or pass `--home /absolute/path` **before** the subcommand to use another location. Files are created with mode `0600`, directories with `0700`. Use the same home on every invocation. The server binds only `127.0.0.1` and may require local-network permission from the Codex sandbox.
...
The existing server hosts one sequential selector worker; there is no additional service. After three quiet seconds following a meaningful authored change (or opted-in visible-message change), it asks one [TypeSafe Choice](https://docs.typesafe.ai/primitives/choice) over at most six existing section candidates plus an unchanged option. The fixed [HTTP API](https://docs.typesafe.ai/api) endpoint is `https://api.typesafe.ai/v1/systemone`, using `jev-latest`. The request includes the authored title, context label/description, current work, outcome, and short candidate excerpts. By default it excludes prompts and feed. With separate `setup.py configure --context on` consent, it includes at most three recent visible user/assistant messages, bounded to 350 UTF-8 bytes each. Tool output, reasoning, history, session IDs, visual HTML, and browser choices remain excluded. Treat authored excerpts as information disclosed to TypeSafe when opting in. Candidate excerpts can be incomplete; later sections outside the first six cannot be selected as the focus.
...
Codex uses a managed startup instruction; no CLI hook trust is required. Start a foreground desktop task and verify its first-turn view. Cursor exposes hook diagnostics in its Hooks tab; Claude Code exposes `/hooks`; OpenCode loads its local plugin after restarting. Installed files alone are not proof of execution.


## Guided setup and configuration

From the repository root, run `python3 session-canvas/setup.py`. Python 3.10+ on local macOS or Linux is required. The wizard does not assume a client configuration, TypeSafe account, or API key already exists. It asks which clients to install, whether to auto-open, and whether to opt into TypeSafe. Review the displayed choices before applying them. Selected host configurations are checked for conflicts before changes; unrelated hooks and skills are preserved.
